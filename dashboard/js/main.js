...
function notifier(status, text, holder) {
	/*
	 * 0=failure 1=success 2=pending
	 */
	if (status == 0) {
		holder.innerHTML = "<span class='alert alert-danger'>" + text
				+ "</span>";
	} else if (status == 1) {
		holder.innerHTML = "<span class='alert alert-success'>" + text
				+ "</span>";
	} else if (status == 2) {
		holder.innerHTML = "<span class='alert alert-info'><span class='fa fa-spinner fa-pulse'></span>"
				+ text + "</span>";
	} else {
...
function addAttribute(obj) {
	...
	for (i = 0; i < attributeNumber; i++) {
		...
		radioLabelTrue.innerHTML = "<input type='radio' name='attr_nullable"
				+ i + "' value='true'>True";

		var radioLabelFalse = document.createElement("label");
		radioLabelFalse.className = "checkbox-inline";
		radioLabelFalse.innerHTML = "<input type='radio' name='attr_nullable"
				+ i + "' value='false'>False";
		...
		radioLabelUniqueTrue.innerHTML = "<input type='radio' name='attr_uniqueness"
				+ i + "' value='true'>True";

		var radioLabelUniqueFalse = document.createElement("label");
		radioLabelUniqueFalse.className = "checkbox-inline";
		radioLabelUniqueFalse.innerHTML = "<input type='radio' name='attr_uniqueness"
				+ i + "' value='false'>False";
...
function loadComboBox(obj) {
	...
	if (obj.value == "select") {
		...
		xmlhttp.open("GET", "../includes/interface.php?action=combo_tables",
				true);
		xmlhttp.send();
		...
		xmlhttp.open("GET",
				"../includes/interface.php?action=combo_table_columns&table_name="
						+ obj.value, true);
		xmlhttp.send();
...
function isDataTypeTable(dataType) {
	var isTable = false;
	if ((dataType != null)
			&& (dataType != "text" && dataType != "password"
					&& dataType != "numeric" && dataType != "date"
					&& dataType != "cloud" && dataType != "file"
					&& dataType != "unique text" && dataType != "long text"
					&& dataType != "select" && dataType != "none")) {
		isTable = true;
	}
	return isTable;
}
...
/**
 * feedEditModal This function is to load the form in the displayed modal
 * 
 * @param {String}
 *            subject the name of the table to update
 * @param {Integer}
 *            occurence_id the id of the instance to be updated
 */
function feedEditModal(subject, occurence_id) {
	notifier(2, " Loading form", document.getElementById("update-notification"));
	var url = "../includes/interface.php?action=feed_modal&caller=site&subject="
			+ subject + "&occurence_id=" + occurence_id;
	var xmlhttp;
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
			...
			if (null !== response) {
				...
				notifier(0, "Internal error", document
